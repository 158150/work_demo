...
from wbfAPI.config import _pushServer
...
class SDK():
    ...
    def start(self):
        targetList = _pushServer.pushServerDict[self.port]
        self.pushServerList = []
        print('websocket实例', targetList)
        for target in targetList:  # 创建ws实例
            exc, symbol = target
        ...
        while not connect:
            try:
                self.zmq = zmq.ZMQ('REP', self.port)
                connect = True
            except:
    ...
    def _dataDrop(self, timeout1=3000, timeout2=5000):
            ...
            try:
                for exc in self.pushServerList:
                    ...
                    if np.array([(current-i[0]) >= timeout1 for i in dataList]).any():
                        exc.restart()  # 重连
                        print(f"{exc} 延迟 {timeout1}ms 触发重连")
                    # 删除latency数据
